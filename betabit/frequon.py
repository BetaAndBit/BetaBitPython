...
pistoale = np.array(messages.pistoale)[0]
lyo = np.array(messages.lyo)[0]
...
def frequon(**args):
    ...
    if "subject" in args and "content" in args:
        ...
        if args["subject"].lower() == subjects["s4"]:
            ...
            h = hs.md5()
            h.update(In.encode())
            if h.hexdigest() == "44752943d3ce60c6756b441fe496f372":
                level = 5
                taskf(level)
            else:
        ...
        if args["subject"].lower() == subjects["s5"]:
            if "attachment" not in args:
                ...
                lengths = ''.join(lengths[6:11])
                h = hs.md5()
                h.update(lengths.encode())
                if h.hexdigest() == "19faa857de6758e79e25bafeb279a252":
        ...
        if args["subject"].lower() == subjects["s6"]: 
            h = hs.md5()
            h.update(args["content"].encode())
            if h.hexdigest() == "652577a1593b22d67a4eb24390301627":
                level = 7
                taskf(level)
            else:
